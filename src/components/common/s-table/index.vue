...
// 下拉菜单相关
const { x, y, showDropdown, menuOption, rightClickRow, onClickoutside, onDropDownSelect } = useDropdown(props.actions);
// 表格相关
const { insColumns, insPagination, getPagination, checkRows, handleCheck } = useDataTable(
  props.pagination,
  props.showCheck,
  props.columns,
  props.actionType,
...
// 请求和数据
const { dataSource, loading, reload } = useRequest(props.request, getPagination);
...
const handlePageChange = (pageSize: number) => {
  insPagination.value.page = pageSize;
  reload();
};
const handleSizeChange = (pageSize: number) => {
  insPagination.value.pageSize = pageSize;
  reload();
};
